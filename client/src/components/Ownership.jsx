...
class Ownership extends Component {
                ...
                this.state.contract.methods
                  ...
                  .call(
                    ...
                    (fileNames, fileHashes) => {
                      ...
                      if (
                        fileHashes !== null &&
                        Object.values(fileHashes)[0].length > 0
                      ) {
                        let userFiles = [];
                        for (
                          let i = 0;
                          i < Object.values(fileHashes)[0].length;
                          i++
                        ) {
                          let tempObject = {};
                          tempObject.id = i + 1;
                          tempObject.fileNames =
                            Object.values(fileHashes)[0][i];
                          tempObject.fileHashes =
                            Object.values(fileHashes)[1][i];
  ...
  checkFileInIpfs = async (event) => {
    ...
    let checkOwnershipForm =
      document.getElementsByName("checkOwnershipForm")[0];

    this.state.contract.methods
      .checkFileExistByHash(this.state.hashToCheck)
      .call((error, val) => {
        if (error) {
          console.log(error);
        }
        if (val[0]) {
          if (val[1] === this.state.activeAccount) {
            alert(
              "Oops, you've stored this file. This file belongs to: " + val[1]
            );
          ...
          <div className="justify-content-between">
            ...
            <div className="px-3 pt-3 d-flex align-items-center justify-content-center">
              <p className="lead text-center">
                Check for file ownership and list your files.
                <br />
                <span className="fs-6">
                  ...
                  section, you can try to upload a file and we will match the
                  hash with all the files stored on our IPFS.
                </span>
              </p>
            </div>
