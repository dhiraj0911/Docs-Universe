...
class Transfer extends Component {
      ...
      try {
        ...
        if (this.state.activeAccount !== accounts[0]) {
          ...
          this.state.contract.methods
            .getFilesByOwner(this.state.activeAccount)
            .call((error, result) => {
              if (error) {
                console.log(error);
              } else {
  ...
  transferOwnership = async (event) => {
    event.preventDefault();
    let transferOwnershipForm = document.getElementsByName(
      "transferOwnershipForm"
    )[0];
    await this.state.contract.methods
      .getFileIdByHash(this.state.ipfsHashTemp)
      .call((err, value) => {
        if (err) {
          console.error(err);
        } else {
          ...
          if (this.state.activeAccount === this.state.recipientAccountTemp) {
            ...
            this.state.contract.methods
              .checkFileExistByHash(this.state.ipfsHashTemp)
              .call((error, owner) => {
                if (error) {
                  console.error(error);
                  transferOwnershipForm.reset();
                  ...
                  if (owner[1] === this.state.activeAccount) {
                    ...
                    this.setState({
                      recipientAccount: this.state.recipientAccountTemp,
                    });
                    this.state.contract.methods
                      ._transfer(
                        this.state.activeAccount,
        ...
        <div className="container">
          <div className="justify-content-between">
            ...
            <div className="px-3 pt-3 d-flex align-items-center justify-content-center">
              <p className="lead text-center">
                ...
                <span className="fs-6">
                  ...
                  <span className="fs-6 fst-italic fw-bold">
                    {" "}
                    Transfer Ownership{" "}
                  </span>
                  section, you can't change anything whose ownership has been
                  transferred, be careful.
                </span>
              </p>
            </div>
            ...
            <div className="pb-3">
              ...
              >
                <div className="mb-3">
                  <label htmlFor="fileHashTransfer" className="form-label">
                    File Hash
                  </label>
                  <input
                    type="text input-block-level"
                    className="form-control"
                    id="fileHashTransfer"
                    onChange={this.handleIpfsHash}
                    required
                  />
                  <div id="fileHashTransferNote" className="form-text">
                    You can only transfer file owned by you.
                  </div>
                </div>
                <div className="mb-3">
                  <label htmlFor="recipientTransfer" className="form-label">
                    Recipient Address
                  </label>
                  <input
                    type="text input-block-level"
                    className="form-control"
                    id="recipientTransfer"
                    onChange={this.handleRecipient}
                    required
                  />
                  <div id="recipientTransferNote" className="form-text">
                    Recipient account address, please recheck.
                  </div>
                </div>
                <div className="mb-3 form-check">
                  ...
                  <label
                    ...
                    htmlFor="transactionCheck"
                  >
                    Check this box if everything is correct.
                  </label>
                </div>
